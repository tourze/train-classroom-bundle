 ...
 * 自动更新过期或需要状态变更的排课记录
 */
#[AsCommand(name: self::NAME, description: '更新排课状态', help: <<<'TXT'
...
final class UpdateScheduleStatusCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $batchSize = is_string($batchSizeOption) ? (int) $batchSizeOption : 100;
    ...
    private function updateScheduledToInProgress(\DateTime $now, int $batchSize, bool $dryRun, SymfonyStyle $io): int
    {
        ...
        if ($count > 0) {
            ...
            foreach ($schedules as $schedule) {
                if (!$schedule instanceof ClassroomSchedule) {
                    continue;
                }
                if (!$dryRun) {
